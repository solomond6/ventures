...
var topStoryBannerSlideshow = function() {
	...
	var $items = $banners.children('.top-story');
	if ($items.length > 1) {
		setTimeout(function() {
			// normalize height.
			var heights = $items.map(function(i, el) { return el.offsetHeight; });
			var maxHeight = Math.max.apply(window, heights);
			var $inners = $items.children('.inner');
			$items.each(function(i, el) {
				var margin = Math.floor((maxHeight - heights[i]) / 2) + 'px';
				$inners[i].style.marginTop = $inners[i].style.marginBottom = margin;
			});
			$banners.slick({
				slide: '.top-story',
				speed: 1000,
...
$('.post-content').find('iframe').each(function(i, el) {
