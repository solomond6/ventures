...
function updateTopStoryBanner(containerNode) {
	...
	if (hasClass(document.body, 'single') && ts.length > 0 && ts[0].getElementsByClassName('thumb').length > 0) {
...
(function($) {
	...
	function initInterviewsOverlay() {
		var url_regexp = /interviews\/?$/,
				video_regexp = /.*vimeo\.com\/(.*)/,
				replacement = '//player.vimeo.com/video/$1?autoplay=1&badge=0&byline=0&color=ea6c11&portrait=0',
				$iframe = $('<iframe webkitallowfullscreen="" mozallowfullscreen="" allowfullscreen="" frameborder="0"></iframe>'),
				$overlay = $('#video-overlay');
		var addcontent = function($link) {
			$overlay.append($iframe.clone().attr('src', $link.attr('data-video-url').replace(video_regexp, replacement)))
	...
	$.fn.initAjaxMailinglist = function() {
		return this.each(function() {
			var $form = $(this);
			var $out = $form.siblings('.mailinglist-out');
			if (!$form.length) return;
			$form.submit(function(e) {
				var $submit = $form.find("input[type=submit]");
				var email   = $.trim($form.find("input[name=ea]").val());

				if (email==="") {
					out('Please enter your email address.');
					e.preventDefault();
					return;
				}
				...
				$.ajax(
					ajaxurl, 
					{
						method: 'POST',
						data: {
							action: 'ventures_mailinglist_join',
							ea: email
						}
					}
				)
			...
			function out(s) {
				if (s) {
					$out.text(s).addClass("active");
				}
				else {
					$out.text("").removeClass("active");
				}
			}
		});
	};
	...
	$.fn.initInfiniteScroll = function() {
		...
		var $post_header = $post_headers().first(),
			...
			$creditlink = $(document.createElement('a')).attr('target', '_blank'),
			$catLabel = null;
		for (var i = 0; i < VENTURES.RELATED_POSTS.length; i++) {
			...
			queue[i].body
				...
				.find('.credit-text').text(item.credit.text).end()
				.find('.credit-link').attr('href', item.credit.url).end()
			...
			if (item.credit.url !== '' && queue[i].body.find('.credit-link').length === 0) {
				queue[i].body.find('.credit-text').wrap($creditlink.clone().attr('href', item.credit.url));
			}
		}
		/**
		 * Scroll to the top before leaving the page, so a refresh leaves us in the
		 * same article we are in.
		 */
		$win.on('beforeunload', function() {
			if (scrollIndex > 0) {
				$target.betterWaypoint('destroy');
				$post_headers().betterWaypoint('destroy');
		...
		$target.betterWaypoint({
			handler: function(direction) {
				...
				if (direction === 'down' && (post = queue.shift())) {
					...
					$win.trigger('resize');
					trackPageView.apply(null, item.page_view);
	...
	$(document).ready(function() {
		...
		(function() {
			if ( !Modernizr.csstransitions || !Modernizr.csstransforms
				|| !('matchMedia' in window && matchMedia("(min-width: 760px)").matches)
			) return;
			var $links = $('a.teaser-content').filter(function() { return this.parentNode.getAttribute('data-full-content'); });
			if ($links.length > 0) {
				$new = $(document.createElement('main')).addClass('site-main new-page').insertAfter($main);
			}
			$links.on('click', function(e) {
				e.preventDefault();
				var link = e.currentTarget, article = e.currentTarget.parentNode;
				$new.html(article.getAttribute('data-full-content')).append($categoryBoxes).transformAdUnits().trigger('contentupdated');
				picturefill({ elements: $new.find('img[sizes]').get() });
				addWatchedElements($new.find('.top-story'), $new.find('.post'));
				updateSharingLinks($new.children('.top-story').data('share_links'));
				$win.trigger('resize');
				pageSwitch.forward(link, function() {
					VENTURES.RELATED_POSTS = JSON.parse(article.getAttribute('data-related-posts'));
					single_post_init();
					trackPageView(link.title, link.href, link.getAttribute('data-post-id'));
				}, true);
			});
		})();
		...
		initAjaxSearch();
		$("form[name=ccoptin]").initAjaxMailinglist();
