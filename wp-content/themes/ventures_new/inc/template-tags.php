...
function ventures_resp_img($post_thumbnail_id, $thumb_data=null) {
	...
	if ($thumb_data) {
		...
		$sizes = array_values($thumb_data['sizes']);
...
function ventures_author_thumbnail($user_id, $echo=true, $user_data=null, $use_article_count=false, $size='medium') {
	...
	$caption = $user_name.($use_article_count ? '<span class="article-count">'.$user_data->article_count.' Articles</span>' : '');
	if ($thumb_id) {
...
function ventures_top_story_banner($post, $mute_category=false, $add_fuoc_script=true) {
	...
	$video_url = $post->post_type === 'ventures_interviews' ? ventures_fetch_interview_video_url() : '';
	$video_desc = $post->post_type === 'ventures_interviews' ? sprintf('<h1>%s</h1>', get_the_title($post)).apply_filters('the_content', ventures_fetch_interview_description($post)) : '';
	...
	if (QUERY_IS_SINGLE_RESULT && $post->post_type === 'ventures_interviews') {
		...
		if (!QUERY_IS_SINGLE_RESULT) {
			$content = sprintf('<a href="%s" rel="bookmark" class="gray-arrow" data-video-url="%s" data-description="%s">%s</a>', esc_url(get_permalink($post)), $video_url, $video_desc, $content);
		}
	}
	if (WP_DEBUG) {
		...
		if ($featured = get_selected_featured_post_id()) {
			...
			$content .= " (Featured post: $post->post_title)";
		}
	}

	if ($add_fuoc_script) $image = str_replace('class="', 'class="blur ', $image);
	print_top_story_banner(array(
		'type' => $post->post_type,
		'bg_color' => $bg_color,
		'bg_pos' => $bg_pos,
	...
	if ($add_fuoc_script) printf('<script src="%s"></script>', get_template_directory_uri().'/js/header-fuoc.js');
}
...
function ventures_post_teaser($post) {
...
function ventures_full_teaser($data, $new_window=false) {
	...
	printf(
		'<article class="%s" data-related-posts="%s" data-full-content="%s">
...
function ventures_featured_image_credit($post_id=0) {
	global $post;
	if ($post_id) $post_id = $post->ID;
	$attpost = get_post(get_post_thumbnail_id($post_id));
	$credit_text = get_media_credit($attpost);
	if (empty($credit_text)) return;
	if ($credit_link = get_media_credit_url($attpost)) {
		$credit_text = sprintf('<a class="credit-link" href="%s" target="_blank">%s</a>', $credit_link, $credit_text);
	}
	printf('<header class="credit"><span>Photograph — %s</span></header>', $credit_text);
}
...
function ventures_narrow_post_teaser($post, $use_thumb=false) {
	...
	if ($is_idea) {
		...
		printf(
			'<article class="%s">
				<a class="teaser-content small-bottom-bar" href="%s" data-video-url="%s">
					<div class="text">
...
function ventures_interview_teaser($post=null) {
	...
	$description = apply_filters('the_content', ventures_fetch_interview_description($post));
	printf(
		'<article class="%s">
			<a class="teaser-content" title="%s" data-post-id="%d" href="%s" data-video-url="%s" data-description="%s" data-share_links="%s">
				<div class="imgwrap fitted-container cover">%s</div>
				<div class="background">&nbsp;</div>
				<span class="text">
					<span class="in">
		...
		esc_url($url),
		$video_url,
		htmlspecialchars("<h1>$title</h1>".$description),
